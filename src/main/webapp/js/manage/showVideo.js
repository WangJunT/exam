...
(function ($) {
    ...
    $(document).on('click','#delete',function () {
        ...
        var that = $(this);
        if (id != undefined) {
            $.get('/SSMDemo/tea/delVideo.action?id='+id,function (data,status) {
                if (status == 'success') {
                    if (data[0] != undefined) {
                        alert('删除成功');
    ...
    function getStu(current,size) {
        $.get('/SSMDemo/tea/selVideo.action?current='+current+'&pageSize='+size+'&reserveFive='+$('#firstSelect').val()+'&reserveSix='+$('#secondSelect').val(),function (data,status) {
            if (status == 'success') {
                // console.log(data);
                    ...
                    if (totalPage > 1) {
                        $('#page').pagination({
                            ...
                            callback: function(current) {
                                // 回调,current(当前页数)
                                $.get('/SSMDemo/tea/selVideo.action?current='+current+'&pageSize='+10+'&reserveFive='+$('#firstSelect').val()+'&reserveSix='+$('#secondSelect').val(),function (data,status) {
                                    if (status == 'success') {
                                        showList(data.dataList);
                                    } else {
                                        alert('发生错误，请稍后重试.');
